...
import type { Sticker } from '../types/studio';
...
interface StickerBuilderProps {
  ...
  onPublishToCommunity: (type: string, data: any) => void;
  navBar?: React.ReactNode;
}

export const StickerBuilder: React.FC<StickerBuilderProps> = ({
  stickers,
  onSaveSticker,
  onDeleteSticker,
  ...
  const [size, setSize] = useState<number>(8); // 8x8 default
  const [isPublishing, setIsPublishing] = useState(false);
  const [isPublished, setIsPublished] = useState(false);
  const [activeWidth, setActiveWidth] = useState<number>(8);
  const [activeHeight, setActiveHeight] = useState<number>(8);
  const [inputW, setInputW] = useState<string>('8');
  const [inputH, setInputH] = useState<string>('8');
  const [stickerName, setStickerName] = useState<string>('My Sticker');
  ...
  // Matrix pixels state
  const [pixels, setPixels] = useState<string[]>(generateBlankGrid(8, 8));
  ...
  useEffect(() => {
    ...
    const onImportJson = () => {
      const fileInput = document.getElementById('sticker-json-import-file') as HTMLInputElement;
      fileInput?.click();
    };
  ...
  }, [pixels, activeColor, drawMode, stickerName, activeWidth, activeHeight, mirrorH, mirrorV, history, redoStack]);
  ...
  const handleCustomSizeApply = () => {
    const w = Math.min(80, Math.max(1, parseInt(inputW) || 8));
    const h = Math.min(16, Math.max(1, parseInt(inputH) || 8));
  ...
  const handleSave = () => {
    ...
    onSaveSticker(newSticker);
    window.showToast('Sticker saved locally!', 'success');
  };

  // Export JSON
  const handleExport = () => {
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(
      JSON.stringify({ name: stickerName, width: currentWidth, height: currentHeight, pixels })
    );
    const downloadAnchor = document.createElement('a');
    downloadAnchor.setAttribute("href", dataStr);
    downloadAnchor.setAttribute("download", `${stickerName.replace(/\s+/g, '_')}_sticker.json`);
      ...
      fileReader.onload = (event) => {
        try {
          ...
          if (parsed.width && parsed.height && parsed.pixels) {
            setHistory(prev => [...prev, pixels]);
            setRedoStack([]);
            setSize(parsed.width === parsed.height ? parsed.width : 0);
            setInputW(parsed.width.toString());
            setInputH(parsed.height.toString());
            setActiveWidth(parsed.width);
            setActiveHeight(parsed.height);
            setStickerName(parsed.name || 'Imported Sticker');
            setPixels(parsed.pixels);
          }
        } catch (err) {
          window.showToast('Invalid JSON sticker format', 'error');
        }
      };
    }
  };
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      await onPublishToCommunity('sticker', shareData);
      setIsPublished(true);
  ...
  const handleExportPNG = () => {
    ...
    link.download = `${stickerName.toLowerCase().replace(/\s+/g, '_') || 'sticker'}_art.png`;
    link.href = canvas.toDataURL('image/png');
    link.click();
  };

  const loadLocalSticker = (st: Sticker) => {
    setHistory(prev => [...prev, pixels]);
    setRedoStack([]);
    setStickerName(st.name);
    setSize(st.width === st.height ? st.width : 0);
    setInputW(st.width.toString());
    setInputH(st.height.toString());
    setActiveWidth(st.width);
    setActiveHeight(st.height);
    setPixels(st.pixels);
  };
  ...
  return (
    <div className="builder-layout-grid">
      ...
      <div className="glass-panel workspace-panel" style={{ height: '100%', overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div>
          ...
          <div className="saved-items-scrollbar-box" style={{ flex: 1, overflowY: 'auto', maxHeight: '420px', marginTop: '10px' }}>
            {stickers.length === 0 ? (
              <p style={{ fontSize: '11px', fontStyle: 'italic', color: 'var(--text-light)', textAlign: 'center', marginTop: '10px' }}>No saved stickers yet.</p>
            ) : (
              stickers.map((st) => (
                <div 
                  key={st.id} 
                  className="saved-asset-card"
                  onClick={() => loadLocalSticker(st)}
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0, flex: 1 }}>
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <p style={{ fontSize: '12px', fontWeight: 'bold', color: 'var(--text-main)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{st.name}</p>
                      <p style={{ fontSize: '9px', color: 'var(--text-light)' }}>{st.width}x{st.height} grid</p>
                    </div>
                  </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '10px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', width: '100%' }}>
              ...
              <label className="btn btn-secondary" style={{ padding: '8px', fontSize: '11px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', margin: 0 }}>
                ...
                <input type="file" accept=".json" onChange={handleImport} style={{ display: 'none' }} id="sticker-json-import-file" />
              </label>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center', width: '100%', marginTop: 'auto', paddingTop: '10px', borderTop: '1px solid rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
              ...
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                <input
                  type="color"
                  value={activeColor.startsWith('#') && activeColor.length === 7 ? activeColor : '#6366f1'}
                  onChange={(e) => {
      ...
      <div className="glass-panel workspace-panel" style={{ height: '100%', overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div>
          <h3 className="panel-header">Sticker Settings</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '11px', color: 'var(--text-light)', fontWeight: '500' }}>Sticker Name</label>
              <input
                type="text"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <label style={{ fontSize: '11px', color: 'var(--text-light)', fontWeight: '500' }}>Grid Size</label>
              <select
                value={size}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: 'auto' }}>
          ...
          >
            <Save size={14} /> Save Sticker <kbd>S</kbd>
          </button>
        </div>
      </div>
